...
model = load_model('C:\\Users\\User.DESKTOP-IIINHE5\\Desktop\\stand_fall4.h5') #model_load
##############################################
...
#YOLO################################
net = cv2.dnn.readNetFromDarknet("C:\\program1\\yolo\\darknet-master\\build\\darknet\\x64\\yolov3.cfg","C:\\program1\\yolo\\darknet-master\\build\\darknet\\x64\\yolov3_last.weights")
...
path_name = "C:\\program1\\influxdb\\influxdb-1.8.3-1\\img\\"
def yolo_detect(frame,count,path_name):
    # forward propogation
    img = cv2.resize(frame, None, fx=0.3, fy=0.3)
        ...
        if i in indexes:
            ...
            if label=="stand":
                ...
                if count %15 == 0:
                    ...
                    ID = model.predict_classes(train_ary)
                    print(ID[-1])
                    #print(type(ID[-1]))
                    if ID[-1] == 0:
                        smtp(img_name,label)
